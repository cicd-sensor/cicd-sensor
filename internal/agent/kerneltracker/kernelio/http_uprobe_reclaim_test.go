...
func newReclaimHarness(t *testing.T) *reclaimHarness {
	t.Helper()
	return &reclaimHarness{worker: newHTTPUprobeWorker(nil, nil, t.TempDir(), httpUprobeWorkerBPF{}, goUprobeTarget{})}
}
...
func TestHTTPUprobeReclaim(t *testing.T) {
	...
	t.Run("queueTargetReconciliation never blocks and keeps one pending sweep", func(t *testing.T) {
		t.Parallel()
		worker := newHTTPUprobeWorker(nil, nil, t.TempDir(), httpUprobeWorkerBPF{}, goUprobeTarget{})
