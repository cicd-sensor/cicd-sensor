...
// issue #48: protected-path persistence rules must match across file_open,
...
func TestPersistenceRulesCoverMoveAndLinkBypass(t *testing.T) {
	...
	cases := []struct {
		...
		// Bypass B: write through a bind-mount alias. path is the alias, but
		// resolved_path carries the canonical location and must still match.
		{"cron_write", celengine.CELInputEvent{Path: "/tmp/x/evil", ResolvedPath: "/etc/cron.d/evil", IsWrite: true}, true},
		{"shell_rc_write", celengine.CELInputEvent{Path: "/tmp/x/rc", ResolvedPath: "/home/runner/.bashrc", IsWrite: true}, true},
		{"ld_so_preload_write", celengine.CELInputEvent{Path: "/tmp/x/p", ResolvedPath: "/etc/ld.so.preload", IsWrite: true}, true},
